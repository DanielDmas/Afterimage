...
### F5 — Event economy is publish-only: `GroundObserved` and `SuspicionEntryAdded` have zero subscribers — **CLOSED**
...
**Post-arc update:** closed via `src/social/ground_observation_bridge.gd` (`GroundObservationBridge`) exactly as suggested, plus the one real design gap the suggestion didn't call out: TruthSim identifies actors by int ID, `SuspicionGraph` identifies NPCs by String id, and nothing anywhere maps between them. The bridge takes that mapping as a constructor argument (an AI actor with no NPC mapping is silently skipped — not every AI needs a social identity) rather than inventing a convention neither system already commits to. `tests/unit/test_ground_observation_bridge.gd` (4 tests) proves the full chain against a real `TruthSim` with a real seeing AI actor: Ground completes → a real `SuspicionLedger` entry lands → `GossipSim` propagates it one real hop to a gossip-connected confidant, arriving on the correct future day. The org-board consumer (rendering `SuspicionGraph`/`GossipSim` state) is still real UI/content work, tracked separately — this closes the *mechanism* gap, not the presentation layer.
...
### F9 — Determinism corpus still points at StubSim (deferred since Pass 3) — **CLOSED**
...
**Post-arc update:** `StubSim` deleted; `tests/fixtures/truth_sim_digest.gd` (`TruthSimDigest`) replaces it, re-simulating each corpus fixture through a real `TruthSim` in the same graybox room `scenes/main.gd` plays in. The three fixtures were rewritten from scratch (not just re-hashed) to exercise real behavior worth guarding: `run_002.json` walks straight into the east wall and keeps pressing past it — real `SweptCollision` clamping must hold identically on every re-simulation — and `run_003.json` runs a full Ground-hold-to-completion cycle (75 ticks), so the digest is sensitive to `GroundState`'s tick-based machine too, not just raw position. Self-consistency (replay twice, compare) stays the verification method, deliberately — Pass 2's own reasoning still holds: hand-writing a "golden" expected digest would risk baking in a hand-computed mistake as the correct answer rather than testing anything real.
