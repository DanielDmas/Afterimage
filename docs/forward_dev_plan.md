...
1. **Real content-driven encounters.** `DriftEncounter`/`PhantomEncounter` are hand-scripted, not `DistortionDirector`/`OpFactory`-driven — that's exactly Phase B below, which supersedes both with real mission content once it lands. This pass made the *current* hand-authored pair good to play; it didn't pull Phase B's timeline forward.
2. **A directional objective indicator.** The HUD states "bottom-right" in text; a real arrow/minimap pointing at the exit would read faster for a player mid-exploration. Small, pure-UI, buildable in-sandbox whenever there's a slot for it.
3. **Procedural audio.** No audio asset pipeline exists in this sandbox, but Godot's `AudioStreamGenerator` can synthesize simple tones/noise *in code*, with no asset files at all — a footstep-adjacent noise floor, a sting when the phantom appears, a tone under the Ground hold. Genuinely buildable here; deferred this pass because it's new, higher-risk API surface (no precedent anywhere in this codebase) that deserved its own dedicated verification-and-test pass rather than being bolted onto an already-large one.
...
6. **Shareable reveal export.** master_plan.md's own marketing beat ("shareable honesty reports") — exporting the reveal panel as an image — is real production/UI work, not a mechanism gap.
Items 2–6 are additive polish, not correctness gaps; nothing above blocks Phase B starting next.

### Phase B — The runtime distortion spine (wire the pipeline into the playable surface) — **PARTIALLY DELIVERED**
*Post-arc update: `src/integration/mission_runtime.gd` (`MissionRuntime`) and `src/integration/mind_model_event_bridge.gd` (`MindModelEventBridge`) are both real and unit-tested — the core mechanism this phase asks for. `scenes/main.gd` has **not** been rewired to use them yet (it still runs `DriftEncounter`/`PhantomEncounter`, the hand-scripted pair from the player-experience "Interlude" pass above), and no determinism-corpus fixture exists for `MissionRuntime` yet. Both are the concrete remaining work — see below, not re-described as if still fully open.*

Replace the hardcoded `DriftEncounter` path with the real one. `MissionRuntime` (delivered) owns: a `MissionPackage` (from `MissionLoader`), a `DistortionDirector` seeded per §4.3, the `MindModel` it reads for budget, and `active_ops: Array[DistortionOp]` (built via `OpFactory.build(package.deck[record["deck_index"]])` from purchase records — `DistortionDirector.purchase_one()`/`authorize_free_tier()` were extended to return `deck_index` in their record, the one piece of information `OpFactory` needs that op-class/tier/cost alone can't disambiguate once a deck ever authors two same-shaped entries with different params). `step(current_tick, ground_just_completed)` is the whole contract: a normal tick attempts one purchase (a no-op most ticks); a Ground-completion tick resolves and clears every active op instead (`notify_ground_resolved`/`notify_op_deactivated`, §4.3's "refund 0, weight decays"). Budget is granted once, at construction, from whatever `MindModel` state the caller passes in — re-granting cadence (once per scene? per day?) is left to the caller, a policy decision this class deliberately doesn't make for a future caller.
- **Augmentation (delivered):** `MindModelEventBridge` wires `TruthSim`'s real `EventBus` facts (`WeaponFired` → `acute_stress.gain_gunfire_in_earshot()`, `ActorDowned` → `acute_stress.gain_witnessing_kill()`, `GroundCompleted` → `acute_stress.relieve_ground_completed()`) into `MindModel` for the first time since Pass 11 shipped it with zero consumers. Deliberately coarse and says so in its own class doc: every event anywhere fires the gain, unconditionally — real earshot-radius gating needs Phase D's `SoundGraph` wiring, and moral injury is deliberately *not* wired from `ActorDowned` here, since §4.4.3's gains are context-weighted (open combat vs. an unaware victim vs. a civilian) and `ActorDowned`'s payload carries none of that context yet; wiring a flat number would be a worse gap than leaving it unwired.
- **Acceptance, delivered half:** a headless test suite (`test_mission_runtime.gd`, `test_mind_model_event_bridge.gd`) asserts (a) same-seed-same-mind-state purchase logs are identical, (b) rising acute stress raises granted budget (measured directly against `DistortionDirector.compute_budget()` as the oracle, matching this codebase's "verify externally, don't hand-assert" discipline), (c) a Ground-completion tick clears every active op and notifies the Director — plus a real-content test that loads `content/missions/m00_stub/mission.json` through the actual `MissionLoader`/`MissionRuntime`/`OpFactory` chain and confirms a real, `FairnessAuditor.KNOWN_OP_CLASSES`-recognized op comes out the other end.
- **Acceptance, still open:** (d) a determinism-corpus fixture for a full `MissionRuntime`-driven run (the same self-consistency discipline `TruthSimDigest` already established — no hand-computed "golden" value, just "replays identically given the same seed"); rewiring `scenes/main.gd` itself to render `MissionRuntime.active_ops` instead of `DriftEncounter`/`PhantomEncounter`'s hand-scripted pair — the player-experience work already invested in those two encounters (the marked exit, the Ground vignette, the Clarity Mode line, the restart loop) needs to survive the swap, not be thrown away; deciding whether the hand-scripted demo and the content-driven runtime coexist (a "tutorial" vs. "real mission" split) or the demo fully migrates to a one-mission `MissionPackage` authored to reproduce the same two-encounter beat.
