...
## Current progress
Passes 1–2 of a planned 20-pass engineering effort are complete: the repo scaffold, the deterministic core (`FixedMath`, `Xoshiro128StarStar` PRNG, `EventBus`, the `Predicate` language + `WorldQuery`), `GameStateStore`/`SaveSystem` with a real schema migration, the fixed-tick harness (`InputFrame`/`ReplayLog`/`FixedTickClock`), a determinism-corpus mechanism (proven via a disposable stub sim ahead of Pass 3's real TruthSim), a custom unit-test harness, and CI are all in place — 111 tests, all passing lint/format. Full detail in [`docs/dev_log.md`](docs/dev_log.md); the live backlog is [`docs/roadmap.md`](docs/roadmap.md).
